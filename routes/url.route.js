...
import { createUrl, deleteId, getAllUserCodes, getUrlByCode, updateUrl } from "../services/user.service.js";
...
router.patch("/:id",ensureAuthenticated,async(req,res)=>{
    ...
    const validationResult = await shortenPostRequestBodySchema.safeParseAsync(
        ...
        const shortCode = code ?? nanoid(6);
        const result = await updateUrl(id,shortCode,url,req.user.id)
        if(!result){
          return res.status(404).json({error:"something went wrong in updating the URL  "})
        }
